...
CUDA_VISIBLE_DEVICES=0,1,2,3 deepspeed --master_port $port src/run_uie.py \
   ...
   --output_dir output/codegen-350M-mono-ner-test \
   --per_device_train_batch_size 6 \
   --per_device_eval_batch_size 16 \
   --gradient_accumulation_steps 6 \
   --learning_rate 1e-05 \
   --num_train_epochs 5 \
   --input_record_file codegen-350M-debug.record \
   --deepspeed configs/ds_configs/stage0.config \
   --run_name codegen-350M-mono-mult-test-experiment \
   --max_source_length 512 \
   --max_target_length 50 \
   --generation_max_length 50 \
   --max_num_instances_per_task 10003 \
